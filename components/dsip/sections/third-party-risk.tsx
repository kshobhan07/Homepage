...
const vendors: Vendor[] = [
  { id: "org", name: "Your organization", tier: 1, x: 560, y: 200, risk: "Low", rating: 100, criticality: 100, incidents: 0, finding: "" },
  { id: "northwind", name: "Northwind Payments", tier: 1, parent: "org", x: 340, y: 70, risk: "Critical", rating: 62, criticality: 88, incidents: 4, finding: "Two unpatched CVEs on internet-facing infrastructure." },
  { id: "cloudline", name: "Cloudline Storage", tier: 1, parent: "org", x: 340, y: 200, risk: "High", rating: 78, criticality: 70, incidents: 1, finding: "Rating improved after MFA enforcement rollout." },
  { id: "vertex", name: "Vertex Logistics", tier: 1, parent: "org", x: 340, y: 330, risk: "High", rating: 69, criticality: 65, incidents: 2, finding: "Subprocessor breach disclosed 11 days ago." },
  { id: "anchor", name: "Anchor HR Systems", tier: 2, parent: "northwind", x: 120, y: 70, risk: "Medium", rating: 58, criticality: 34, incidents: 3, finding: "Credential leak affecting admin portal access." },
  { id: "helio", name: "Helio Analytics", tier: 2, parent: "cloudline", x: 120, y: 200, risk: "Medium", rating: 84, criticality: 40, incidents: 0, finding: "No open findings — reassessed quarterly." },
  { id: "beacon", name: "Beacon Legal Suite", tier: 2, parent: "vertex", x: 120, y: 330, risk: "Low", rating: 91, criticality: 18, incidents: 0, finding: "Low blast radius, strong security posture." },
];
  ...
  return (
    <Panel className="overflow-hidden !rounded-[18px] p-0 lg:col-span-2">
      <div className="flex items-center justify-between border-b border-white/[.07] px-5 py-3.5">
        ...
        <Tag tone="amber">Risk propagation</Tag>
      </div>
          ...
          {vendors.map((v) => {
            ...
            return (
              <g key={v.id} onMouseEnter={() => v.id !== "org" && onHover(v)} className={v.id !== "org" ? "cursor-pointer" : undefined}>
                <GraphNode x={v.x} y={v.y} width={170} height={46} color={v.id === "org" ? "#2f6fed" : tierColor[v.risk]}>
...
function VendorDetail({ vendor }: { vendor: Vendor }) {
  return (
    <Panel className="flex flex-col justify-between p-5">
      <div>
        <p className="font-mono text-[10px] uppercase tracking-wide text-white/40">Selected vendor</p>
        <div className="mt-1 flex items-center gap-2">
          <h3 className="font-display text-lg font-semibold text-white">{vendor.name}</h3>
          <Tag tone={riskTone[vendor.risk]}>{vendor.risk}</Tag>
        </div>
        <p className="mt-3 text-sm leading-6 text-white/60">{vendor.finding}</p>
      </div>
  ...
  return (
    <section id="third-party" className="mx-auto max-w-[1400px] px-5 py-24 sm:px-8">
      <SectionHeading
        ...
        tone="amber"
        title="Every vendor, mapped by how its risk reaches you"
        description="A living dependency graph, not a spreadsheet — hover a vendor to trace the propagation path back to your organization."
      />
