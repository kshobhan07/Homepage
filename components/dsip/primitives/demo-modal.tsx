...
import { Calendar, Check, ChevronDown, Loader2, Sparkles, X } from "lucide-react";
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block font-mono text-[10px] uppercase tracking-wide text-white/40">{label}</span>
      {children}
    </label>
  );
}
...
const inputClass =
  "h-11 w-full rounded-lg border border-white/12 bg-white/[.04] px-3.5 text-sm text-white placeholder:text-white/30 focus:border-signal-blue/50 focus:outline-none";

function SelectField({ label, options, required = true }: { label: string; options: string[]; required?: boolean }) {
  return (
    <Field label={label}>
      <div className="relative">
        <select required={required} defaultValue="" className={cn(inputClass, "appearance-none pr-9 text-white/85")}>
    ...
    <form onSubmit={handleSubmit} className="p-6 sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Preferred date">
          <div className="relative">
            <input type="date" className={cn(inputClass, "pr-9 [color-scheme:dark]")} />
